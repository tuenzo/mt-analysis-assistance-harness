...
- [x] **T1.1** — 创建 `backend/app/agent/session_store.py`
  ```python
  class SessionStore:
...
- [x] **T1.3** — 创建 `backend/app/agent/context_builder.py`
  ...
  ```
- [x] **T1.4** — 创建 `backend/app/agent/prompt_composer.py`
  ```python
...
- [x] **T2.1** — 定义 `ClaudeRuntimeAdapter` 接口（ABC）
  ```python
  ...
  ```
- [x] **T2.2** — 创建 `MockClaudeRuntimeAdapter`
  ```python
  ...
  ```
- [x] **T2.3** — 添加配置：`config.py` 的 `agent_runtime.provider`（mock | claude_agent_sdk）

### Phase 3: Message Runtime 主循环

- [x] **T3.1** — 创建 `backend/app/agent/message_runtime.py`
  ```python
  ...
  ```
- [x] **T3.2** — 工具调用路由：当收到 `tool_call_started` 事件时，需要识别 action 并转发到 AnalysisToolGateway（暂不实现 gateway，先 mock tool result 返回）

### Phase 4: Agent Messages API

- [x] **T4.1** — 创建 `backend/app/api/agent_messages.py`
  ```python
  ...
  ```
- [x] **T4.2** — `AgentMessageRequest` schema
  ```python
  ...
  ```
- [x] **T4.3** — 注册到 `main.py` 的 `app.include_router(agent_messages.router, prefix="/api")`

### Phase 5: Session 管理 API

- [x] **T5.1** — 创建 `GET /api/projects/{project_id}/sessions` — 列出项目的 sessions
- [x] **T5.2** — 创建 `GET /api/agent/sessions/{session_id}` — 获取 session 详情

### Phase 6: 前端改造（参考）

- [x] **T6.1** — 前端 sendMessage 不再调用 `/api/agent/runs`，改为 `POST /api/agent/messages`
- [x] **T6.2** — 前端 SSE 监听 `/api/agent/sessions/{session_id}/events`，渲染事件到 Agent Command Center
- （前端改动不在本 change 内，参考实现见 spec）

### Phase 7: 测试

- [x] **T7.1** — `test_message_runtime.py` — 发送消息返回 turn_id
- [x] **T7.2** — `test_sse_events.py` — 事件流正确推送
- [x] **T7.3** — `test_mock_adapter.py` — mock 返回模拟事件
- [x] **T7.4** — `test_interrupt.py` — interrupt 正确停止 session
- [x] **T7.5** — 运行所有测试，修复问题
