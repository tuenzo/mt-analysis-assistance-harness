...
- [x] **T1.1** — 创建 `backend/app/analysis/pipelines/build_panel.py` — 数据校验部分
  ```python
  ...
  ```
- [x] **T1.2** — 创建 `validate_files()` 函数，读取 CSV header 检查必要字段
- [x] **T1.3** — 在 `data_tools.py` 中接入 `validate_files()`

### Phase 2: Panel Build (panel.build_category_day)

- [x] **T2.1** — 实现 `build_category_day_panel(project_id: str) -> PanelResult`
  ```python
  ...
  ```
- [x] **T2.2** — 注册 panel_tools.py 中的 `panel_build_category_day()`
- [x] **T2.3** — 单元测试：`test_build_panel.py`

### Phase 3: Diagnostics (analysis.run_diagnostics)

- [x] **T3.1** — 实现 `run_diagnostics(project_id: str) -> DiagnosticsResult`
  - GMV trend（按日期折线图数据）
  - activity vs non-activity 统计
  - payday overlap 分析
  - category concentration top10
- [x] **T3.2** — 输出 JSON 结果到 `.analysis/diagnostics_result.json`
- [x] **T3.3** — 注册到 analysis_tools.py

### Phase 4: LocalGap (analysis.run_localgap)

- [x] **T4.1** — 实现 `run_localgap(project_id: str) -> LocalGapResult`
  ```python
  ...
  ```
- [x] **T4.2** — 注册到 analysis_tools.py

### Phase 5: PSM-DID (analysis.run_psm_did)

- [x] **T5.1** — 实现简化版 `run_psm_did(project_id: str) -> PSMDIDResult`（第一版可先做简化，不支持复杂协变量）
- [x] **T5.2** — 注册到 analysis_tools.py

### Phase 6: GPS-Uplift (analysis.run_gps_uplift) — Stub

- [x] **T6.1** — 创建 stub 实现（返回 method_status="stub"，生成占位文件）
  ```python
...
- [x] **T7.1** — 创建 `backend/app/analysis/pipelines/full_pipeline.py`
  ```python
  ...
  ```
- [x] **T7.2** — 注册到 analysis_tools.py

### Phase 8: Chart Render

- [x] **T8.1** — 创建 `backend/app/analysis/chart_renderer.py`
  ```python
  def render_key_charts(project_id: str) -> list[Artifact]:
      ...
      # 注册 artifact
  ```
- [x] **T8.2** — 注册到 chart_tools.py

### Phase 9: Job Orchestrator 接入

- [x] **T9.1** — 创建 `backend/app/jobs/orchestrator.py`
  ```python
  class JobOrchestrator:
      def submit_job(self, project_id: str, action: str, input: dict) -> Job:
          ...
          # 返回 job_id
  ```
- [x] **T9.2** — 创建 `backend/app/jobs/runner.py`
  ```python
  class JobRunner:
...
- [x] **T10.1** — `test_validate.py` — 缺少文件返回正确错误
- [x] **T10.2** — `test_build_panel.py` — 生成 panel.parquet
- [x] **T10.3** — `test_diagnostics.py` — 输出 trend 数据
- [x] **T10.4** — `test_localgap.py` — 输出分解结果和图表
- [x] **T10.5** — `test_full_pipeline.py` — 端到端测试
- [x] **T10.6** — 运行所有测试，修复问题
