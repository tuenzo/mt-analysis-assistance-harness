...
- [ ] **T1.1** — 创建 `backend/app/agent/claude_agent_sdk_adapter.py`
  ```python
  ...
  ```
- [ ] **T1.2** — 配置 `config.py` 的 `agent_runtime` 部分：
  ```python
...
- [ ] **T4.1** — `external_session_id` 持久化到 `analysis_sessions.external_session_id`
- [ ] **T4.2** — `resume_session` 时用 stored external_session_id 恢复
- [ ] **T4.3** — session 状态同步（active/paused/completed/interrupted/error）

### Phase 5: Hooks & Permissions

- [ ] **T5.1** — 配置 permission mode
  ```python
  permissionMode = "dontAsk"  # MVP 默认不询问
  ```
- [ ] **T5.2** — 工具调用前 hooks（可记录审计日志）
  ```python
  def before_tool_call(tool_name, parameters):
      log_tool_call(tool_name, parameters)
  ```
- [ ] **T5.3** — 可选：添加 `canUseTool` callback 自定义权限逻辑

### Phase 6: 文档与配置

- [ ] **T6.1** — 创建 `docs/CLAUDE_ADAPTER_SETUP.md`
  ```
  ...
  ```
- [ ] **T6.2** — 添加优雅 fallback：没有 API key 时自动降级到 mock

### Phase 7: 测试

- [ ] **T7.1** — `test_claude_adapter_interface.py` — 验证接口一致性（mock 和 real 实现同一接口）
- [ ] **T7.2** — `test_claude_adapter_streaming.py` — streaming 模式正确 yield events（需要 api key，标记为 integration test）
- [ ] **T7.3** — `test_session_persistence.py` — external_session_id 正确存储和恢复
- [ ] **T7.4** — 运行测试，修复问题
